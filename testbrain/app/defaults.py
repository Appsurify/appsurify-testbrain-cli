...
from testbrain.utils.functional import memoize
...
OLD_NS = {'testbrain_{0}'}
OLD_NS_BEAT = {'testbrainbeat_{0}'}
OLD_NS_WORKER = {'testbraind_{0}'}
...
class Option:
    """Describes a Testbrain configuration option."""
...
NAMESPACES = Namespace(
    ...
    beat=Namespace(
        ...
        scheduler=Option('testbrain.beat:PersistentScheduler'),
        schedule_filename=Option('testbrainbeat-schedule'),
    ...
    cache=Namespace(
        __old__=old_ns('testbrain_cache'),
    ...
    azureblockblob=Namespace(
        container_name=Option('testbrain', type='string'),
    ...
    control=Namespace(
        ...
        exchange=Option('testbrain', type='string'),
    ),
    couchbase=Namespace(
        __old__=old_ns('testbrain_couchbase'),

        backend_settings=Option(None, type='dict'),
    ),
    arangodb=Namespace(
        __old__=old_ns('testbrain_arangodb'),
        backend_settings=Option(None, type='dict')
    ),
    mongodb=Namespace(
        __old__=old_ns('testbrain_mongodb'),

        backend_settings=Option(type='dict'),
    ),
    cosmosdbsql=Namespace(
        database_name=Option('testbraindb', type='string'),
        collection_name=Option('testbraincol', type='string'),
        consistency_level=Option('Session', type='string'),
        max_retry_attempts=Option(9, type='int'),
        max_retry_wait_time=Option(30, type='int'),
    ),
    event=Namespace(
        __old__=old_ns('testbrain_event'),

        queue_expires=Option(60.0, type='float'),
        queue_ttl=Option(5.0, type='float'),
        queue_prefix=Option('testbrainev'),
        serializer=Option('json'),
        exchange=Option('testbrainev', type='string'),
    ),
    redis=Namespace(
        __old__=old_ns('testbrain_redis'),
    ...
    result=Namespace(
        __old__=old_ns('testbrain_result'),

        backend=Option(type='string'),
        cache_max=Option(
            -1,
            type='int', old={'testbrain_max_cached_results'},
        ),
        compression=Option(type='str'),
        exchange=Option('testbrainresults'),
        exchange_type=Option('direct'),
        expires=Option(
            timedelta(days=1),
            type='float', old={'testbrain_task_result_expires'},
        ),
    ...
    elasticsearch=Namespace(
        __old__=old_ns('testbrain_elasticsearch'),
    ...
    security=Namespace(
        __old__=old_ns('testbrain_security'),
    ...
    database=Namespace(
        url=Option(old={'testbrain_result_dburi'}),
        engine_options=Option(
            type='dict', old={'testbrain_result_engine_options'},
        ),
        short_lived_sessions=Option(
            False, type='bool', old={'testbrain_result_db_short_lived_sessions'},
        ),
        table_schemas=Option(type='dict'),
        table_names=Option(type='dict', old={'testbrain_result_db_tablenames'}),
    ),
    task=Namespace(
        __old__=OLD_NS,
        ...
        compression=Option(type='string', old={'testbrain_message_compression'}),
        create_missing_queues=Option(True, type='bool'),
        inherit_parent_priority=Option(False, type='bool'),
        default_delivery_mode=Option(2, type='string'),
        default_queue=Option('testbrain'),
        ...
        eager_propagates=Option(
            False, type='bool', old={'testbrain_eager_propagates_exceptions'},
        ),
        ignore_result=Option(False, type='bool'),
        store_eager_result=Option(False, type='bool'),
        protocol=Option(2, type='int', old={'testbrain_task_protocol'}),
        publish_retry=Option(
            True, type='bool', old={'testbrain_task_publish_retry'},
        ),
        publish_retry_policy=Option(
            {'max_retries': 3,
             ...
             'interval_max': 1,
             'interval_step': 0.2},
            type='dict', old={'testbrain_task_publish_retry_policy'},
        ),
        ...
        send_sent_event=Option(
            False, type='bool', old={'testbrain_send_task_sent_event'},
        ),
        serializer=Option('json', old={'testbrain_task_serializer'}),
        soft_time_limit=Option(
            type='float', old={'testbraind_task_soft_time_limit'},
        ),
        time_limit=Option(
            type='float', old={'testbraind_task_time_limit'},
        ),
    ...
    worker=Namespace(
        ...
        autoscaler=Option('testbrain.worker.autoscale:Autoscaler'),
        cancel_long_running_tasks_on_connection_loss=Option(
            False, type='bool'
        ),
        concurrency=Option(None, type='int'),
        consumer=Option('testbrain.worker.consumer:Consumer', type='string'),
        direct=Option(False, type='bool', old={'testbrain_worker_direct'}),
        disable_rate_limits=Option(
            False, type='bool', old={'testbrain_disable_rate_limits'},
        ),
        deduplicate_successful_tasks=Option(
            False, type='bool'
        ),
        enable_remote_control=Option(
            True, type='bool', old={'testbrain_enable_remote_control'},
        ),
        hijack_root_logger=Option(True, type='bool'),
        log_color=Option(type='bool'),
        log_format=Option(DEFAULT_PROCESS_LOG_FMT),
        lost_wait=Option(10.0, type='float', old={'testbraind_worker_lost_wait'}),
        ...
        redirect_stdouts=Option(
            True, type='bool', old={'testbrain_redirect_stdouts'},
        ),
        redirect_stdouts_level=Option(
            'WARNING', old={'testbrain_redirect_stdouts_level'},
        ),
        send_task_events=Option(
            False, type='bool', old={'testbrain_send_events'},
        ),
        state_db=Option(),
...
def find_deprecated_settings(source):  # pragma: no cover
    from testbrain.utils import deprecated
...
def find(name, namespace='testbrain'):
