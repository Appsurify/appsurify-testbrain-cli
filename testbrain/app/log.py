...
The Testbrain instances logging section: ``Testbrain.log``.
...
from testbrain import signals
from testbrain.exceptions import CDeprecationWarning, CPendingDeprecationWarning
from testbrain.local import class_property
from testbrain.platforms import isatty
from testbrain.utils.log import (ColorFormatter, LoggingProxy, get_logger,
                                 get_multiprocessing_logger, mlevel,
                                 reset_multiprocessing_logger)
from testbrain.utils.nodenames import node_format
from testbrain.utils.term import colored
...
class Logging:
    ...
    def setup(self, loglevel=None, logfile=None, redirect_stdouts=False,
        ...
        os.environ.update(
            TESTBRAIN_LOG_LEVEL=str(loglevel) if loglevel else '',
            TESTBRAIN_LOG_FILE=str(logfile) if logfile else '',
        )
    ...
    def redirect_stdouts(self, loglevel=None, name='testbrain.redirected'):
        self.redirect_stdouts_to_logger(
            get_logger(name), loglevel=loglevel
        )
        os.environ.update(
            TESTBRAIN_LOG_REDIRECT='1',
            TESTBRAIN_LOG_REDIRECT_LEVEL=str(loglevel or ''),
        )
    ...
    def setup_logging_subsystem(self, loglevel=None, logfile=None, format=None,
        ...
        if not receivers:
            ...
            if self.app.conf.worker_hijack_root_logger:
                root.handlers = []
                get_logger('testbrain').handlers = []
                get_logger('testbrain.task').handlers = []
                get_logger('testbrain.redirected').handlers = []
    ...
    def get_default_logger(self, name='testbrain', **kwargs):
