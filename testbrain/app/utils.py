...
from testbrain.exceptions import ImproperlyConfigured
from testbrain.platforms import pyimplementation
from testbrain.utils.collections import ConfigurationView
from testbrain.utils.imports import import_from_cwd, qualname, symbol_by_name
from testbrain.utils.text import pretty
...
BUGREPORT_INFO = """
software -> testbrain:{testbrain_v} kombu:{kombu_v} py:{py_v}
...
class Settings(ConfigurationView):
    ...
    def broker_read_url(self):
        return (
            os.environ.get('TESTBRAIN_BROKER_READ_URL') or
    ...
    def broker_write_url(self):
        return (
            os.environ.get('TESTBRAIN_BROKER_WRITE_URL') or
    ...
    def broker_url(self):
        return (
            os.environ.get('TESTBRAIN_BROKER_URL') or
    ...
    def result_backend(self):
        return (
            os.environ.get('TESTBRAIN_RESULT_BACKEND') or
            self.first('result_backend', 'TESTBRAIN_RESULT_BACKEND')
        )
    ...
    def find_option(self, name, namespace=''):
        ...
        Example:
            >>> from proj.testbrain import app
    ...
    def find_value_for_key(self, name, namespace='testbrain'):
    ...
    def finalize(self):
        # See PendingConfiguration in testbrain/app/base.py
    ...
    def maybe_warn_deprecated_settings(self):
        # TODO: Remove this method in Testbrain 6.0
        if self.deprecated_settings:
            from testbrain.app.defaults import _TO_NEW_KEY
...
def bugreport(app):
    ...
    return BUGREPORT_INFO.format(
        ...
        testbrain_v=testbrain.VERSION_BANNER,
    ...
    if isinstance(sym, ModuleType) and ':' not in app:
        try:
            ...
            try:
                ...
                if isinstance(found, ModuleType):
                    raise AttributeError(
                        "attribute 'testbrain' is the testbrain module not the instance of testbrain")
            except AttributeError:
                ...
                for suspect in vars(sym).values():
                    if isinstance(suspect, Testbrain):
                        return suspect
                raise
            else:
                return found
