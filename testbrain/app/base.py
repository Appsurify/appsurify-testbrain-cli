...
from testbrain import platforms, signals
from testbrain._state import (_announce_app_finalized, _deregister_app,
                              _register_app, _set_current_app, connect_on_app_finalize,
                              get_current_app, set_default_app)
from testbrain.exceptions import AlwaysEagerIgnored, ImproperlyConfigured
from testbrain.loaders import get_loader_cls
from testbrain.local import PromiseProxy, maybe_evaluate
from testbrain.utils.collections import AttributeDictMixin
from testbrain.utils.dispatch import Signal
from testbrain.utils.functional import first, head_from_fun, maybe_list
from testbrain.utils.imports import instantiate, symbol_by_name
from testbrain.utils.log import get_logger
from testbrain.utils.objects import FallbackContext, mro_lookup
from testbrain.utils.time import maybe_make_aware, timezone, to_utc
...
from .utils import (AppPickler, Settings, _new_key_to_old, _old_key_to_new,
                    _unpickle_app, _unpickle_app_v2, appstr, detect_settings)
...
def app_has_custom(app, attr):
    ...
    """
    return mro_lookup(app.__class__, attr, stop={Testbrain, object},
...
class Testbrain:
    ...
    Keyword Arguments:
        ...
        backend (Union[str, Type[testbrain.backends.base.Backend]]):
    ...
    events_cls = None
    loader_cls = None
    log_cls = 'testbrain.app.log:Logging'
    control_cls = 'testbrain.app.control:Control'
    task_cls = None
    registry_cls = None
    ...
    def __init__(self, main=None, loader=None, backend=None,
        ...
        # this attribute is set in testbrain.worker.trace and checked by testbrain.worker.request
    ...
    def _get_default_loader(self):
        ...
        return (
            os.environ.get('TESTBRAIN_LOADER') or
            self.loader_cls or
            'testbrain.loaders.app:AppLoader'
        )
    ...
    def close(self):
        ...
        Example:
            >>> with Testbrain(set_as_current=False) as app:
    ...
    def start(self, argv=None):
        """Run :program:`testbrain` using `argv`.

        Uses :data:`sys.argv` if `argv` is not specified.
        """
        from testbrain.bin.testbrain import testbrain

        testbrain.params[0].default = self

        if argv is None:
            argv = sys.argv

        try:
            testbrain.main(args=argv, standalone_mode=False)
        except Exit as e:
            return e.exit_code
        finally:
            testbrain.params[0].default = None

    def worker_main(self, argv=None):
        """Run :program:`testbrain worker` using `argv`.
    ...
    def register_task(self, task, **options):
        ...
        Note:
            This is here for compatibility with old Testbrain 1.0
    ...
    def config_from_object(self, obj,
        ...
        Example:
            >>> testbrain.config_from_object('myapp.testbrainconfig')

            >>> from myapp import testbrainconfig
            >>> testbrain.config_from_object(testbrainconfig)
    ...
    def config_from_envvar(self, variable_name, silent=False, force=False):
        ...
        Example:
            >>> os.environ['TESTBRAIN_CONFIG_MODULE'] = 'myapp.testbrainconfig'
            >>> testbrain.config_from_envvar('TESTBRAIN_CONFIG_MODULE')
        """
    ...
    def config_from_cmdline(self, argv, namespace='testbrain'):
        self._conf.update(
    ...
    def setup_security(self, allowed_serializers=None, key=None, cert=None,
        ...
        """
        from testbrain.security import setup_security
    ...
    def signature(self, *args, **kwargs):
        """Return a new :class:`~testbrain.Signature` bound to this app."""
        kwargs['app'] = self
