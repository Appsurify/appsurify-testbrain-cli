"""The ``testbrain shell`` program, used to start a REPL."""
...
def shell(ctx, ipython=False, bpython=False,
          python=False, eventlet=False, gevent=False):
    """Start shell session with convenient access to testbrain symbols.

    The following symbols will be added to the main globals:
    - ``testbrain``:  the current application.
    - all registered tasks.
    """
    sys.path.insert(0, os.getcwd())
    if eventlet:
        import_module('testbrain.concurrency.eventlet')
    if gevent:
        import_module('testbrain.concurrency.gevent')
    import testbrain
    ctx.obj.echo(f'Testbrain {testbrain.VERSION_BANNER}')
    app = ctx.obj.app
