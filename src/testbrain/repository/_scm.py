...
class SourceCodeManager:
    ...
    def get_all_tags(cls) -> List[str]:
        ...
        try:
            result = subprocess.run(
                cls._ALL_TAGS_COMMAND, text=True, check=True, capture_output=True
            )  # noqa: S603
    ...
    @classmethod
    def tag_in_scm(
        cls, config: "Config", context: MutableMapping, dry_run: bool = False
    ) -> None:
...
class Git(SourceCodeManager):
    ...
    def latest_tag_info(cls, tag_pattern: str) -> SCMInfo:
        ...
        try:
            ...
            subprocess.run(
                ["git", "update-index", "--refresh", "-q"], capture_output=True
            )  # noqa: S603, S607
        ...
        try:
            ...
            result = subprocess.run(
                git_cmd, text=True, check=True, capture_output=True
            )  # noqa: S603
        ...
        info = SCMInfo(
            tool=cls, branch_name=branch_name, short_branch_name=short_branch_name
        )
...
class Mercurial(SourceCodeManager):
    ...
    def latest_tag_info(cls, tag_pattern: str) -> SCMInfo:
        ...
        is_dirty = (
            len(subprocess.check_output(["hg", "status", "-mard"])) != 0
        )  # noqa: S603, S607
    ...
    def assert_nondirty(cls) -> None:
        ...
        if lines:
            joined_lines = b"\n".join(lines).decode()
            raise DirtyWorkingDirectoryError(
                f"Mercurial working directory is not clean:\n{joined_lines}"
            )
